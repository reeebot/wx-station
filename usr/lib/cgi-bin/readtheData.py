...
with open('temperature.php', 'w') as temperaturefile:
	temperaturefile.write('<p><p><p1>&nbsp;%0.0f&deg;<br></p1>' % (Fahrenheit))
	temperaturefile.write('<p2><b>%0.0f%% RH</b><br></p2>' % (humidity)) #%0.1f&deg;F Dewpoint (realDP)
	temperaturefile.write('<p3>{0:0.2f} inHg</p3><p><p><p><p>'.format(inhg))

with open('tempdata.txt', 'w', os.O_NONBLOCK) as datafile:
    datafile.write('{0:0.1f}'.format(Fahrenheit) + "\n" + '{0:0.1f}'.format(realDP) + "\n" + '{0:0.0f}'.format(humidity) + "\n" + '{0:0.2f}'.format(inhg) + "\n")
